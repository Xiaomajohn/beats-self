...
import (
	...
	"github.com/elastic/beats/v7/auditmetricbeat/metricbeat/module/haproxy"
...
func eventMapping(info *haproxy.Info, r mb.ReporterV2) (mb.Event, error) {
	...
	for i := 0; i < st.NumField(); i++ {
		...
		if typeOfT.Field(i).Name == "IdlePct" {
			...
			if err != nil {
				return mb.Event{}, fmt.Errorf("error getting IdlePct: %w", err)
			}
			...
			if err != nil {
				r.Error(err)
				return mb.Event{}, fmt.Errorf("error getting Memmax_MB: %w", err)
			}
	...
	if err != nil {
		return event, fmt.Errorf("error applying schema: %w", err)
	}
